...
js/
  ...
  bullet.js         砲彈狀態與繪製
  powerup.js        寶物：+1 命 / 限時加速 / 限時護盾，敵人擊毀機率掉落
...
| 寶物 | 圖示 | 效果 |
|------|------|------|
| 醫療補給 | 綠十字 | +1 生命（上限 5；滿命時改給 500 分） |
| 加速 | 青色雙箭頭 | 8 秒內移速 ×1.45（車尾顯示氣流特效） |
| 護盾 | 藍色盾牌 | 6 秒無敵（與重生無敵同款護盾圈） |

加速在死亡重生後清除；護盾與現有無敵時間取較長者，不疊加。
...
- 寶物系統（+1 命 / 加速 / 護盾），機率掉落、限時存在、撿取音效與浮字回饋
...
- 更多道具（強化砲彈可破鋼牆、鏟子加固基地、炸彈全滅、星星升級）
- 多張地圖／關卡編輯器、隨機地圖模式
- 雙人同樂（第二組鍵位）
- 高分榜（localStorage）
- 觸控／手把支援
